...
litellm-cli keyinfo
litellm-cli teams
litellm-cli team-rank
```

查看团队排行（包含 Key 排行和用量排行）：

```bash
litellm-cli team-rank          # 进入 TUI 界面
litellm-cli team-rank --help   # 查看快捷键
```

快捷键：
- `Tab` - 切换 Key 排行 / 用量排行
- `1-5` - 用量排行时间范围（最近7天/30天/3个月/半年/今年）
- `o` - 用量排行排序切换（Token / 请求数）
- `↑↓` - 移动选择
- `enter` - 查看详情
- `esc` - 返回
- `←→` - 切换 Tab
